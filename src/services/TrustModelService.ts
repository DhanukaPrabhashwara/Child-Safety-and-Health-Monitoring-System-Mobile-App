...
import SupabaseService, { VoiceLog } from './SupabaseService';
...
class TrustModelService {
    ...
    async checkAudio(testAudioPath: string, childId?: string): Promise<TrustCheckResult> {
        let keyring: Record<string, number[][]> = {};

        // 1. Load Local Keyring
        const storedData = await AsyncStorage.getItem(TRUSTED_VOICES_KEY);
        if (storedData) {
            keyring = JSON.parse(storedData);
        }

        // 2. Load Cloud Keyring (Watch App data)
        if (childId) {
            try {
                const cloudVoices = await SupabaseService.getVoicesForChild(childId);
                if (cloudVoices.length > 0) {
                    keyring['Watch (Cloud)'] = cloudVoices.map(v => v.voice_vector);
                }
            } catch (err) {
                console.error('Failed to fetch cloud voices for checkAudio', err);
            }
        }
        ...
        for (const [personName, vectors] of Object.entries(keyring)) {
            ...
            for (const enrolledVector of vectors) {
                ...
                // Ensure dimensions match. The Watch app currently saves 192 dims, 
                // but the Mobile app model outputs 128. 
                // For a robust system, these must be aligned. We'll handle slicing for now.
                const minLength = Math.min(enrolledVector.length, testEmbedding.length);

                // Cosine Similarity
                let dotProduct = 0;
                for (let i = 0; i < minLength; i++) {
                    dotProduct += enrolledVector[i] * testEmbedding[i];
                }
    ...
    async isEnrolled(childId?: string): Promise<boolean> {
        const stored = await AsyncStorage.getItem(TRUSTED_VOICES_KEY);
        let hasLocal = false;
        if (stored) {
            const keyring = JSON.parse(stored);
            hasLocal = Object.keys(keyring).length > 0;
        }

        if (hasLocal) return true;

        if (childId) {
            const voices = await SupabaseService.getVoicesForChild(childId);
    ...
    async getEnrolledVoices(childId?: string): Promise<{ name: string; sampleCount: number; audioPaths: string[] }[]> {
        let results: { name: string; sampleCount: number; audioPaths: string[] }[] = [];
        ...
        if (storedData) {
            ...
            results = Object.keys(keyring).map(name => ({
                name,
        ...
        // 2. Add Cloud Voices if childId is provided
        if (childId) {
            try {
                const cloudVoices = await SupabaseService.getVoicesForChild(childId);
                if (cloudVoices.length > 0) {
                    results.push({
                        name: 'Watch (Cloud)',
                        sampleCount: cloudVoices.length,
                        audioPaths: []
                    });
                }
            } catch (err) {
                console.error('Failed to fetch cloud voices for UI', err);
            }
        }
